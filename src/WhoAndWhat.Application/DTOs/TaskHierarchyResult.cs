...
public class TaskHierarchyResult
{
    ...
    public static TaskHierarchyResult CreateSimple(DomainTask rootTask)
    {
        return new TaskHierarchyResult
        {
            ...
            TotalTaskCount = 1,
            MaxDepth = 0,
            CompletedTaskCount = rootTask.Status == (int)Domain.ValueObjects.TaskStatus.Completed ? 1 : 0,
            CompletionPercentage = rootTask.Status == (int)Domain.ValueObjects.TaskStatus.Completed ? 100m : 0m,
...
public class TaskHierarchyNode
{
    ...
    public void CalculateMetrics()
    {
        if (!Subtasks.Any())
        {
            ...
            SubtreeCompletionPercentage = Task.Status == (int)Domain.ValueObjects.TaskStatus.Completed ? 100m : 0m;
            return;
        }
        ...
        CompletedDescendantCount = Subtasks.Sum(s => s.CompletedDescendantCount + 
            (s.Task.Status == (int)Domain.ValueObjects.TaskStatus.Completed ? 1 : 0));
        
        var totalTasks = DescendantCount + 1; // +1 for this task
        var completedTasks = CompletedDescendantCount + 
            (Task.Status == (int)Domain.ValueObjects.TaskStatus.Completed ? 1 : 0);
