...
public sealed class TimeBlockManager : ITimeBlockManager
{
        ...
        List<SchedulingPattern> patterns, CancellationToken cancellationToken)
    {
        ...
        {
            return Task.FromResult<TimeBlockSuggestion?>(null);
        }
        ...
        return Task.FromResult<TimeBlockSuggestion?>(new TimeBlockSuggestion(
            Id: Guid.NewGuid(),
    ...
    CancellationToken cancellationToken)
{
    ...
    return Task.FromResult(strategicBlocks);
}

private Task<TimeBlockSuggestion> OptimizeIndividualTimeBlockAsync(
    ...
    CancellationToken cancellationToken)
{
    ...
    if (optimalTime.HasValue && Math.Abs((optimalTime.Value - block.StartTime).TotalHours) > 1)
    {
        ...
        {
            return Task.FromResult(block with
            {
