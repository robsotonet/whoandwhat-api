...
public record ArchiveCriteria
{
    ...
    /// </summary>
    public TaskStatus[] ArchivableStatuses { get; init; } = 
    {
    ...
    public bool ShouldArchiveTask(Task task, DateTime currentTime)
    {
        // Check if task status is archivable
        if (!ArchivableStatuses.Contains((TaskStatus)task.Status))
        {
            return false;
        }
        ...
        var requiredAge = task.Status == (int)TaskStatus.Completed 
            ? MinimumCompletedAge 
            : MinimumCanceledAge;
        ...
        if (!IncludeParentTasks && task.Subtasks.Any(s => s.Status != (int)TaskStatus.Completed))
        {
            return false;
        }
