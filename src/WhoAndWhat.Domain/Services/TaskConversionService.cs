...
public class TaskConversionService
{
    ...
    public (ValidationResult ValidationResult, DomainTask? ConvertedProject) ConvertTaskToProject(DomainTask task, IEnumerable<DomainTask>? subtasks = null)
    {
        ...
        var convertedTask = new DomainTask
        {
            Id = task.Id,
            Title = task.Title,
            ...
            Category = TaskCategory.Project.Value,
            Status = task.Status,
    ...
    private ValidationResult ValidateTaskToProjectConversion(DomainTask task, IEnumerable<DomainTask>? subtasks = null)
    {
        ...
        var currentCategory = TaskCategory.FromValue(task.Category);

        // Use the task's own business logic
        if (!task.CanConvertToProject())
        {
            errors.Add("Task cannot be converted to project based on current state");
        }

        // Additional validation for specific scenarios
        if (currentCategory == TaskCategory.Appointment)
        {
            errors.Add("Appointments cannot be converted to projects");
        }

        if (currentCategory == TaskCategory.BillReminder)
        {
    ...
    public (ValidationResult ValidationResult, DomainTask? ConvertedTask) ConvertTaskCategory(DomainTask task, TaskCategory targetCategory)
    {
        var currentCategory = TaskCategory.FromValue(task.Category);
        ...
        var convertedTask = CreateConvertedTask(task, targetCategory);
    ...
    /// </summary>
    private ValidationResult ValidateCategoryConversion(DomainTask task, TaskCategory fromCategory, TaskCategory toCategory)
    {
        var errors = new List<string>();
        ...
        if (fromCategory == TaskCategory.Idea && toCategory == TaskCategory.Appointment)
        {
        ...
        if (fromCategory == TaskCategory.Idea && toCategory == TaskCategory.BillReminder)
        {
            if (!task.DueDate.HasValue)
            {
    ...
    /// </summary>
    private DomainTask CreateConvertedTask(DomainTask originalTask, TaskCategory newCategory)
    {
        var convertedTask = new DomainTask
        {
            Id = originalTask.Id,
        ...
        if (newCategory == TaskCategory.Appointment)
        {
        ...
        if (newCategory == TaskCategory.BillReminder)
        {
    ...
    /// </summary>
    private int AdjustPriorityForCategory(int currentPriorityValue, TaskCategory newCategory)
    {
    ...
    {
        var category = TaskCategory.FromValue(projectTask.Category);
        ...
        foreach (var template in subtaskTemplates)
        {
            ...
            var subtask = new DomainTask
            {
                ...
                Category = TaskCategory.ToDo.Value, // Subtasks are typically ToDo items
    ...
    private string EnhanceDescriptionForProject(string? originalDescription, DomainTask originalTask)
    {
        ...
        {
            var conversionNote = $"\n\n--- Project converted from {TaskCategory.FromValue(originalTask.Category).GetDisplayName()} on {DateTime.UtcNow:yyyy-MM-dd} ---";
    ...
    public (TaskCategory SuggestedCategory, double ConfidenceScore) SuggestTaskCategory(string title, string? description, DateTime? dueDate)
    {
        var scores = new Dictionary<TaskCategory, double>();
        ...
        scores[TaskCategory.Appointment] += CountKeywords(content, appointmentKeywords) * 0.3;
        scores[TaskCategory.BillReminder] += CountKeywords(content, billKeywords) * 0.3;
        scores[TaskCategory.Project] += CountKeywords(content, projectKeywords) * 0.3;
        scores[TaskCategory.Idea] += CountKeywords(content, ideaKeywords) * 0.3;
        ...
        if (dueDate.HasValue)
        {
            ...
            if (daysUntilDue <= 1)
            {
                scores[TaskCategory.Appointment] += 0.4; // Likely an appointment
                scores[TaskCategory.BillReminder] += 0.3; // Could be a bill
            }
            else if (daysUntilDue > 30)
            {
                scores[TaskCategory.Project] += 0.2; // Long-term could be project
                scores[TaskCategory.Idea] += 0.1; // Or long-term idea
            }
        }
        else
        {
            scores[TaskCategory.Idea] += 0.2; // No due date suggests idea
        }
        ...
        if (totalLength > 200)
        {
            scores[TaskCategory.Project] += 0.2; // Detailed descriptions suggest projects
        }
        else if (totalLength < 50)
        {
            scores[TaskCategory.ToDo] += 0.2; // Short descriptions suggest simple todos
            scores[TaskCategory.Idea] += 0.1;
        }
